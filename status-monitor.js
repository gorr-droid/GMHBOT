...
// Exact Supabase REST endpoint from your DevTools Network tab
...
async function fetchProductsFromDatabase() {
  try {
    ...
    if (!res.ok) {
      console.error(`[STATUS MONITOR] Supabase returned HTTP ${res.status}: ${res.statusText}`);
    ...
    const parsed = data.map(item => {
      // Products with null tool_status default to ONLINE on gmh-shop.com
      let status = 'ONLINE';
    ...
    console.log(`[STATUS MONITOR] Loaded ${parsed.length} tools directly from Supabase.`);
    return parsed;
  } catch (err) {
...
module.exports = {
  startMonitoring: (client) => {
    ...
    // Sync database every 60 seconds
    cron.schedule('* * * * *', () => {
      runStatusCheck(client);
    });
  },
